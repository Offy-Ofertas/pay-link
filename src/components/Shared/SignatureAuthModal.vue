    ...
    <v-card>
      <v-card-title class="d-flex align-center justify-space-between">
        <div class="d-flex align-center gap-2">
          ...
          <span class="font-weight-medium">Autenticação facial</span>
        </div>
        <v-chip
          :color="chip.color"
      ...
      <v-card-text>
        <p class="text-body-2 mb-4">
          Posicione seu rosto no enquadramento e clique para capturar. Após a
          autenticação, o botão de assinatura será liberado.
        </p>
        ...
        <div class="face-grid">
          ...
          <div class="face-actions">
            <v-btn
              ...
              block
              :loading="ligandoCamera"
              :disabled="faceVerificando"
              @click="iniciarCamera()"
            >
            ...
            >
              ...
              Capturar e autenticar
            </v-btn>

            <div class="status mt-3" :class="`status-${statusColor}`">
              <v-icon :color="statusColor" size="18" class="mr-1">
                {{ statusIcon }}
              </v-icon>
...
<script setup>
  ...
  const props = defineProps({
    ...
    colaboradorId: { type: String, default: "" },
    cpf: { type: String, default: "" },
    alwaysApprove: { type: Boolean, default: true },
  });
  ...
  const statusMensagem = computed(() => {
    switch (faceStatus.value) {
      case "aprovado":
        return "Rosto validado. Liberando a assinatura.";
      case "reprovado":
  ...
  async function capturarEValidar() {
    ...
    try {
      ...
      if (!props.colaboradorId) {
        if (props.alwaysApprove) {
          finalizarAutenticacao();
        } else {
          faceStatus.value = "reprovado";
        }
        return;
      }
