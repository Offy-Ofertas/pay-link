...
async function salvar() {
    if (!form.value.nome || !form.value.cpf || !form.value.cargo || !form.value.salario) {
        admin.showSnackbar("Preencha nome, CPF, cargo e salário.", "warning");
        return;
    }
    ...
    try {
        const payload = {
            ...form.value,
            salario: salarioParaNumero(form.value.salario),
        };

        if (isEdit.value) {
            await colaboradorStore.atualizar(route.params.id, payload);
            admin.showSnackbar("Colaborador atualizado com sucesso!", "success");
        } else {
            await colaboradorStore.criar(payload);
...
function mascararSalario() {
    const valor = (form.value.salario || "").replace(/\D/g, "");
